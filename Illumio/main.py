...
class IllumioPlugin(PluginBase):
    ...
    def _get_headers_for_auth(
        self, api_path: str, auth_username: str, auth_password: str, request_type: str
    ) -> Dict:
        """Return header for authentication.

        Args:
            api_path (str): API path string.
            auth_username(str): Illumio API Access ID.
            auth_password(str): Illumio API Secret Key.
        ...
        bytes_auth_password = bytes(auth_password, "utf-8")

        # HMAC-SHA256
        dig = hmac.new(
            bytes_auth_password, msg=bytes_api_path, digestmod=hashlib.sha256
        ).digest()

        # BASE64 ENCODE
        hmac_sha256 = base64.b64encode(dig).decode()
        signature = f"TC {auth_username}:{hmac_sha256}"
        header = {
    ...
    def get_pull_request(self, api_url):
        ...
        query_endpoint = self.configuration["api_url"] + api_url
    ...
    def pull_data_from_Illumio(
        self,
        api_path: str,
        threat_type: str,
        tagging: bool,
    ) -> List[Indicator]:
        """Fetch Data from Illumio API.
        ...
        while True:
            ...
            if ioc_response_json.get("status", "") != "Success":
                raise requests.exceptions.HTTPError(
                    f"Plugin: Illumio, Unable to fetch Indicator. "
    ...
    def _api_calls(self, request):
        ...
        try:
            ...
            self.logger.error(
                f"Plugin: Illumio, Connection Error occurred: {err}."
            )
            raise err
            ...
            self.logger.error(
                f"Plugin: Illumio, Exception occurred: {err}."
            )
            raise err

    def _is_valid_credentials(
        self, api_url: str, auth_username: str, auth_password: str
    ) -> bool:
        """Validate credentials.

        Args:
            auth_username (str): Access ID for Illumio.
            auth_password (str): Secret Key for Illumio.

        Returns:
            bool: True for valid credentials and false for not valid.
        """
        api_path = "/api/v3/security/owners"
        query_endpoint = api_url + api_path
        headers = self._get_headers_for_auth(
            api_path,
            auth_username,
            auth_password,
    ...
    def validate(self, configuration: Dict) -> ValidationResult:
        ...
        if (
            ...
            or "Illumio" not in configuration["api_url"].split(".")
        ):
            self.logger.error(
                "Illumio Plugin: "
    ...
    def pull(self) -> List[Indicator]:
        ...
        if self.configuration["is_pull_required"] == "Yes":
            ...
            self.logger.info(
                "Plugin: Illumio Polling is disabled, skipping."
            )
            return []
    ...
    def get_group_id(self, action_dict):
        ...
        if action_dict.get("parameters").get("group_name") != "create_group":
            ...
            create_group_api = self.configuration["api_url"] + api_path
            headers = self._get_headers_for_auth(
                api_path,
                self.configuration["auth_username"],
                self.configuration["auth_password"],
    ...
    def update_ioc(self, value, group_id):
        ...
        url = self.configuration["api_url"] + api_path
        headers = self._get_headers_for_auth(
            api_path,
            self.configuration["auth_username"],
            self.configuration["auth_password"],
    ...
    def push(self, indicators: List[Indicator], action_dict: Dict):
        """Push Indicators to Illumio Platform.
        ...
        api_path = "/api/v3/indicators"
        query_endpoint = self.configuration["api_url"] + api_path
        invalid_ioc = 0
        already_exists = 0
        for indicator in indicators:
            ...
            if (
                ...
                self.logger.error(
                    f"Error while pushing IoCs to Illumio. "
                    f"Error: {response.json()['message']}."
                )
                return PushResult(
                    success=False,
                    message="Error while pushing IoCs to Illumio.",
                )
        if invalid_ioc != 0:
            self.logger.error(
        ...
        if already_exists != 0:
            self.logger.warn(
                f"Updated {already_exists} IoC(s) on Illumio."
            )
        return PushResult(
            success=True,
            message="Indicators pushed successfully to Illumio.",
        )
    ...
    def get_owner(self):
        ...
        endpoint = self.configuration["api_url"] + api_path
    ...
    def get_group_names(self) -> Dict:
        ...
        if owner_name:
            ...
            url = self.configuration.get("api_url") + api_path
            group_names = {}
                ...
                if response.json()["status"] == "Success":
                    ...
                    if response.json().get("next", None):
                        api_path = (
                            response.json()
                            .get("next")
                            .replace(self.configuration["api_url"], "")
                        )
